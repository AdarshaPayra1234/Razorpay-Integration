...
app.patch('/:id/status', authenticateAdmin, async (req, res) => {
  try {
    ...
    const statusUpdateHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #00acc1;">Booking Status Update</h2>
...
app.patch('/:id/reschedule', authenticateAdmin, async (req, res) => {
  try {
    const { newWeddingDate, newPreWeddingDate, reason } = req.body;
    const bookingId = req.params.id;

    if (!newWeddingDate) {
      return res.status(400).json({ error: 'Wedding date is required' });
    }

    const updates = {
      bookingDates: newWeddingDate,
      ...(newPreWeddingDate && { preWeddingDate: newPreWeddingDate }),
      $push: {
        updates: {
          date: new Date(),
          type: 'reschedule',
          details: {
            reason,
            oldDates: {
              wedding: booking.bookingDates,
              preWedding: booking.preWeddingDate
            },
            newDates: {
              wedding: newWeddingDate,
              preWedding: newPreWeddingDate
            }
          }
        }
      }
    ...
    const rescheduleHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #00acc1;">Booking Rescheduled</h2>
        <p>Dear ${booking.customerName},</p>
        <p>Your booking (ID: ${booking._id}) has been rescheduled:</p>
        <ul>
          <li><strong>New Wedding Date:</strong> ${new Date(newWeddingDate).toLocaleDateString()}</li>
          ${newPreWeddingDate ? `<li><strong>New Pre-Wedding Date:</strong> ${new Date(newPreWeddingDate).toLocaleDateString()}</li>` : ''}
        </ul>
...
app.put('/:id', authenticateAdmin, async (req, res) => {
  try {
    const bookingId = req.params.id;
...
app.post('/:id/payment', authenticateAdmin, async (req, res) => {
  try {
    ...
    const paymentHtml = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <h2 style="color: #00acc1;">Payment Received</h2>
        <p>Dear ${booking.customerName},</p>
        <p>We have received a payment of ₹${amount} for your booking (ID: ${booking._id}).</p>
    ...
    res.json({ 
      ...
      booking: {
        ...
        payments: booking.paymentBreakdown.payments
      }
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
app.get('/:id', authenticateAdmin, async (req, res) => {
  try {
    ...
    const paidAmount = booking.paymentBreakdown?.advancePaid || 0;
    ...
    const responseData = {
      ...
      preWeddingDate: booking.preWeddingDate || 'Not specified'
    };
...
app.get('/stats', authenticateAdmin, async (req, res) => {
  try {
    ...
    const totalRevenue = allBookings.reduce((sum, booking) => {
      const price = booking.package ? parseInt(booking.package.replace(/[^0-9]/g, '')) || 0 : 0;
      return sum + price;
    }, 0);
    
    const paidRevenue = allBookings.reduce((sum, booking) => {
      return sum + (booking.paymentBreakdown?.advancePaid || 0);
    }, 0);
